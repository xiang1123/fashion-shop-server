...
def add_item(req: CartItemCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if item:
        ...
        item = CartItem(cart_id=cart.id, sku_id=req.sku_id, quantity=req.quantity, created_at=datetime.now(), updated_at=datetime.now())
        db.add(item)
    db.commit()
    return ok(True)

@router.patch("/cart/items/{item_id}")
def update_item(item_id: int, req: CartItemUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    cart = get_or_create_cart(db, user.id)
